...
class RectifiedFlow(Module):
    ...
    def predict_flow(self, model: Module, noised, cond, *, times, eps = 1e-10):
        """
        ...
        output = self.model(noised, cond=cond, **model_kwargs)
    ...
    def sample(
        ...
        **model_kwargs
        ...
        cond = repeat(cond, "1 s -> n s", n=batch_size)
        def ode_fn(t, x):
            x = maybe_clip(x)

            _, flow = self.predict_flow(model, x, cond, times = t, **model_kwargs)
    ...
    def forward(
        ...
        **model_kwargs
        ...
        def get_noised_and_flows(model, t):
            ...
            model_output, pred_flow = self.predict_flow(model, noised, cond, times = t)
